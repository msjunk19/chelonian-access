...
//             // Log command with source (P = Proximity, M = Manual)
//             LogSource logSrc = (source == "P") ? LogSource::PROXIMITY : LogSource::BLE;
//             const char* cmdMsg = (command == 1) ? "Unlock" : (command == 2) ? "Lock" : (command == 5) ? "Trunk" : (command == 6) ? "Panic" : "Command";
//             accessLogger.logAccess(logSrc, LogResult::SUCCESS, deviceId.c_str(), cmdMsg);
...
//     //     void onRead(NimBLECharacteristic* pChar, NimBLEConnInfo& connInfo) override {
//     //         // On read, return the next chunk based on current offset
//     //         ESP_LOGI(BLETAG, "Log onRead: sending chunk at offset %d", _offset);
//     //         String json = accessLogger.getLogsJsonChunk(-1, _offset, 3);
            
//     //         if (json.isEmpty() || json == "[]") {
//     //             // No more logs at this offset, reset for next client
//     //             ESP_LOGI(BLETAG, "No more logs at offset %d, resetting", _offset);
//     //             _offset = 0;
//     //             pChar->setValue("[]");
//     //         } else {
//     //             // Set the value and increment offset for next read
//     //             pChar->setValue(json.c_str());
//     //             _offset += 3;
                
//     //             // Reset if we've gone past all logs
//     //             if (_offset >= LOG_MAX_ENTRIES) {
//     //                 _offset = 0;
//     //             }
//     //             ESP_LOGI(BLETAG, "Sent chunk, next offset will be: %d", _offset);
//     //         }
//     //     }
 
//     //     void onWrite(NimBLECharacteristic* pChar, NimBLEConnInfo& connInfo) override {
//     //         std::string raw = pChar->getValue();
//     //         size_t offset = atoi(raw.c_str());
            
//     //         ESP_LOGI(BLETAG, "Log onWrite: offset=%zu", offset);
            
//     //         String json = accessLogger.getLogsJsonChunk(-1, offset, 3);
...
//     //         pChar->setValue(json.c_str());
//     //         pChar->notify();
//     //         ESP_LOGI(BLETAG, "Notified chunk at offset %zu", offset);
//     //     }
...
//         ESP_LOGI(BLETAG, "Log request offset=%d", offset);
...
//         ESP_LOGI(BLETAG, "NOTIFY SENT %d bytes", json.length());

//         // 🔥 IMPORTANT: allow iOS subscription to settle
//         delay(80);

//         pChar->setValue(json.c_str());
//         pChar->notify();

//         ESP_LOGI(BLETAG, "Sent chunk size=%d", json.length());
//     }
...
//     //     ) override
//     //     {
//     //         ESP_LOGI(BLETAG, "Log onRead triggered");

//     //         String json = accessLogger.getLogsJson(-1, 0, 10);

//     //         ESP_LOGI(BLETAG, "Sending logs: %s", json.c_str());
