...
const String VERIFY_UUID      = "beb54843-36e1-4688-b7f5-ea07361b26a8";
...
class _HomePageState extends State<HomePage> {
  ...
  BluetoothCharacteristic? _verifyChar;
  ...
  void _showErrorDialog(String title, String message, {bool clearPairing = false}) {
    if (!mounted) return;
    showDialog(
      context: context,
          ...
          if (clearPairing)
            TextButton(
              onPressed: () async {
                Navigator.pop(ctx);
                await _clearPairing();
                setState(() { _status = "Pairing cleared — please re-pair"; });
              },
...
Future<void> _loadPairing() async {
  ...
  Future<void> _connectToDevice(BluetoothDevice device) async {
    ...
    try {
      ...
      for (BluetoothService service in services) {
        if (service.uuid.toString() == SERVICE_UUID) {
          for (BluetoothCharacteristic c in service.characteristics) {
            ...
            if (uuid == VERIFY_UUID)      _verifyChar               = c;
          }
        }
      }
      ...
      // Verify pairing on connect
      if (_paired && _deviceId != null) {
        await _verifyPairing();
      }
      ...
      _showErrorDialog("Connection Failed", 
          "Could not connect to Chelonian: $e");
    }
  }
...
// }
  ...
  Future<void> _pair() async {
    ...
    try {
      ...
      if (token.startsWith("error:")) {
        _showErrorDialog("Pairing Failed",
            "Could not pair: ${token.replaceFirst('error:', '').replaceAll('_', ' ')}");
      ...
      _showErrorDialog("Pairing Error", "An error occurred while pairing: $e");
      setState(() { _status = "Pairing error"; });
    }
  }
  ...
  Future<void> _sendCommand(int command) async {
    ...
    if (!_connected || _cmdChar == null) {
      await _scanAndConnect();
      if (!_connected) {
        _showErrorDialog("Not Connected",
            "Could not connect to Chelonian. Make sure you are in range.");
        return;
      }
    }
    ...
    try {
      ...
      _showErrorDialog("Command Failed",
  ...
  Future<void> _startProximityMonitoring() async {
    ...
    try {
      ...
      _showErrorDialog("Beacon Error",
          "Could not start proximity monitoring: $e");
      setState(() { _proximityStatus = "Beacon error"; });
    }
  }
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    if (_lastAction.isNotEmpty && !_lastAction.startsWith("error:")) ...[
