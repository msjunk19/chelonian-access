...
void initState() {
  ...
  SharedPreferences.getInstance().then((prefs) {
    _securityManager = SecurityManager(prefs);
  });
  ...
  Future<void> _scanAndConnect() async {
    ...
    if (state != BluetoothAdapterState.on) {
      ...
      _appState.update(scanning: false, status: "Bluetooth is off");  // ← add this
      return;
    }
...
Future<String?> _fetchNonce() async {
  if (!_connected) {
    setState(() { _status = "Not connected"; });
    _appState.update(status: "Not connected");
    return null;
  }

  // Rate limit nonce requests (1 per 500ms)
  if (!_securityManager.checkRateLimit('nonce', 
      maxRequestsPerWindow: 5, windowSeconds: 1)) {
    setState(() { _status = "Too many requests"; });
    _appState.update(status: "Too many requests");
    return null;
  }

  try {
    // For BLE, we would normally request nonce from device
    // For now, generate locally (device would validate the nonce)
    final nonce = _securityManager.generateLocalNonce();
    debugPrint("Generated nonce: $nonce");
    return nonce;
  } catch (e) {
    debugPrint("Failed to fetch nonce: $e");
    setState(() { _status = "Failed to get nonce"; });
    return null;
  }
}
